...
main()
{
  ...
  show_day_month=$(get_tmux_option "@dracula-day-month" true)
  show_refresh=$(get_tmux_option "@dracula-refresh-rate" 5)
  IFS=' ' read -r -a plugins <<< $(get_tmux_option "@dracula-plugins" "battery time")
  ...
  for plugin in "${plugins[@]}"; do
    ...
    if [ $plugin = "time" ]; then
      ...
      if $show_day_month && $show_military ; then # military time and dd/mm
        ...
        script="%d/%m %I:%M %p ${timezone} "
      else
        script="%m/%d %I:%M %p ${timezone} "
      fi
    fi
